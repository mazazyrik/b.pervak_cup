    ...
    <div className='p-0'>
      ...
      <div className='p-4'>
        ...
        {isLoading && (
          <div className='flex flex-col gap-3'>
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className='w-full h-40 bg-neutral-900 animate-pulse rounded-xl' />
            ))}
          </div>
        )}
        {isError && (
        ...
        {!!items && items.length > 0 && (
          <div className='flex flex-col gap-3'>
            {items.map((p, idx) => (
              <div key={p.id} className='rounded-xl overflow-hidden border border-white/10 bg-black/30 card-appear' style={{ animationDelay: `${idx * 30}ms` }}>
                ...
                <div className='p-3 flex items-center justify-between gap-3'>
                  ...
                  <div className='flex items-center gap-2'>
                    <button disabled={approve.isPending} onClick={() => approve.mutate(p.id)} className='px-4 py-2 rounded-full bg-white text-black text-sm button-pop disabled:opacity-60'>Одобрить</button>
                    <button disabled={remove.isPending} onClick={() => remove.mutate(p.id)} className='px-4 py-2 rounded-full bg-neutral-800 text-white text-sm button-pop disabled:opacity-60' style={{ animationDelay: '60ms' }}>Удалить</button>
                  </div>
                </div>
              </div>
            ))}
